...
def set_recall(building_id: int, body: RecallRequest, db: Session = Depends(get_db)):
    ...
    if body.active:
        ...
        # 轿厢空载留守召回层，方向复位为 idle，与召回期间的“驶向召回层”区分
        car_rows = db.scalars(
            select(ElevatorCar).where(ElevatorCar.building_id == b.id)
        ).all()
        for c in car_rows:
            c.floor = b.recall_floor
            c.load = 0
            c.direction = "idle"
...
def create_call(body: CallCreate, db: Session = Depends(get_db)):
    ...
    if not b:
        raise HTTPException(404, "楼栋不存在")
    if b.recall_active:
        raise HTTPException(409, "消防召回中：禁止登记新呼梯")
...
def dispatch(body: DispatchRequest, db: Session = Depends(get_db)):
    ...
    if building.recall_active or ticket.status == "frozen":
        raise HTTPException(409, "消防召回中：呼梯冻结，禁止派工")
